    ...
    async query (sql, args) {
        ...
        return new Promise((resolve, reject) => {
            ...
            conn.query(sql, args, (err, result) => {
                        if (err) {
                            console.log(err)
                            resolve({errno:err.errno,code:err.code,sqlMessage:err.sqlMessage})
                        } else {
                            resolve(result)
                        }
                        conn.end()
 ...
 */
    async commonSelect ( args ) {
        let {tables, fields, wheres, sorts, pageIndex, pageSize} = args;
        if(tables == '' || tables == undefined){
            return {code:-1,codeMsg:'argument "tables" not be a null value'}
        }else{
            let where      = (wheres ? wheres + ' and is_delete=0' : 'is_delete=0');
            let sql        = 'SELECT '         +
                (fields    ? fields:'*')       +
                ' FROM '   + tables + ' where ' + 
                where      + ' order by '      +
            ...
            if(total.length >= 0 && list.length >= 0){
                ...
                return {code:list.errno || total.errno,codeMsg:list.code || total.code }
            }   
        }
    },
    async setLog( args ){
        let {uid,ped_operation,operation_code,operation_msg,api_url} = args
